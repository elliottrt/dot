...
std::vector<token::group_t> generate_groups(const token::group_t &tokens, token::token_type delim = token::token_type::DOT) {
	...
	// start of group
	token::group_t::const_iterator group_start = tokens.cbegin();
	// current position
	token::group_t::const_iterator iter = tokens.cbegin();
	...
	for (; iter != tokens.cend(); ++iter) {
		...
		if (tok->type == delim && stack.empty()) {
			if (iter == group_start)
				throw SyntaxError(tok->loc, "expected expression, found nothing");
